...
def go_to(url, session_id=None):
    ...
    Opens the specified URL in the current web browser window. For instance::
        ...
        go_to("google.com")
    """
    _get_api_impl(session_id).go_to_impl(url)


def set_driver(driver, session_id=None):
    """
    Sets the Selenium WebDriver used to execute Helium commands. See also
    :py:func:`get_driver`.
    """
    _get_api_impl(session_id).set_driver_impl(driver)
...
def get_driver(session_id=None):
    ...
    """
    return _get_api_impl(session_id).get_driver_impl()
...
def press(key, session_id=None):
    ...
    example, to press Control + a, call::
        ...
        press(CONTROL + 'a')
    """
    _get_api_impl(session_id).press_impl(key)
...
def click(element, session_id=None):
    """
...
:py:class:`selenium.webdriver.remote.webelement.WebElement` or :py:class:`Point`
    ...
    """
    _get_api_impl(session_id).click_impl(element)


def doubleclick(element, session_id=None):
    """
    :param element: The element or point to click.
    :type element: str, unicode, :py:class:`HTMLElement`, \
:py:class:`selenium.webdriver.remote.webelement.WebElement` or :py:class:`Point`
    ...
    """
    _get_api_impl(session_id).doubleclick_impl(element)
...
:py:class:`selenium.webdriver.remote.webelement.WebElement` or :py:class:`Point`
    ...
    """
    _get_api_impl(session_id).drag_impl(element, to)


def press_mouse_on(element, session_id=None):
    _get_api_impl(session_id).press_mouse_on_impl(element)


def release_mouse_over(element, session_id=None):
    _get_api_impl(session_id).release_mouse_over_impl(element)


def find_all(predicate, session_id=None):
    """
    ...
    """
    return _get_api_impl(session_id).find_all_impl(predicate)


def scroll_down(num_pixels=100, session_id=None):
    """
    Scrolls down the page the given number of pixels.
    """
    _get_api_impl(session_id).scroll_down_impl(num_pixels)


def scroll_up(num_pixels=100, session_id=None):
    """
    Scrolls the the page up the given number of pixels.
    """
    _get_api_impl(session_id).scroll_up_impl(num_pixels)


def scroll_right(num_pixels=100, session_id=None):
    """
    Scrolls the page to the right the given number of pixels.
    """
    _get_api_impl(session_id).scroll_right_impl(num_pixels)


def scroll_left(num_pixels=100, session_id=None):
    """
    Scrolls the page to the left the given number of pixels.
    """
    _get_api_impl(session_id).scroll_left_impl(num_pixels)


def hover(element, session_id=None):
    """
...
:py:class:`selenium.webdriver.remote.webelement.WebElement` or :py:class:`Point`
    ...
    """
    _get_api_impl(session_id).hover_impl(element)


def rightclick(element, session_id=None):
    """
    :param element: The element or point to click.
    :type element: str, unicode, :py:class:`HTMLElement`, \
:py:class:`selenium.webdriver.remote.webelement.WebElement` or :py:class:`Point`
    ...
    """
    _get_api_impl(session_id).rightclick_impl(element)


def select(combo_box, value, session_id=None):
    """
    ...
    """
    _get_api_impl(session_id).select_impl(combo_box, value)


def drag_file(file_path, to, session_id=None):
    """
    ...
    """
    _get_api_impl(session_id).drag_file_impl(file_path, to)


def attach_file(file_path, to=None, session_id=None):
    """
    ...
    finds on the page.
    """
    _get_api_impl(session_id).attach_file_impl(file_path, to=to)
...
def refresh(session_id=None):
    ...
    closes it.
    """
    _get_api_impl(session_id).refresh_impl()


def wait_until(condition_fn, timeout_secs=10, interval_secs=0.5, session_id=None):
    """
    ...
    """
    _get_api_impl(session_id).wait_until_impl(condition_fn, timeout_secs, interval_secs)
...
class GUIElement:
    def __init__(self, session_id=None):
        self._driver = _get_api_impl(session_id).require_driver()
        self._args = []
...
class HTMLElement(GUIElement):
    def __init__(
            self, below=None, to_right_of=None, above=None, to_left_of=None,session_id=None
    ):
        super(HTMLElement, self).__init__(session_id=session_id)
...
class Window(GUIElement):
    ...
    def __init__(self, title=None,session_id=None):
        super(Window, self).__init__(session_id=session_id)
...
def switch_to(window, session_id=None):
    """
...
:py:class:`Window` object
    ...
    """
    _get_api_impl(session_id).switch_to_impl(window)


def kill_browser(session_id=None):
    """
    ...
    automation run::
        ...
        kill_browser()
    """
    _get_api_impl(session_id).kill_browser_impl()
...
def highlight(element, session_id=None):
    ...
    """
    _get_api_impl(session_id).highlight_impl(element)
...
def _is_session_valid(session_id=None):
    ...
    try:
        if session_id in _API_IMPL:
            ...
            num_handles = len(driver.window_handles) > 0
            if num_handles == 0:
